...
barx <- barplot(yy, col=c("gray90", "gray30"), main="", xlab="Amino Acid", ylab="Density", beside=T,ylim=c(0,ee.up[which.max(yy)] + max(yy)), names.arg=aalist.sh)
...
print(ee)
barx <- barplot(yy, col=c("gray90", "gray30"), main="", xlab="Amino Acid", ylab="Density", beside=T,ylim=c(0,ee[which.max(yy)] + max(yy)), names.arg=aalist.sh)
